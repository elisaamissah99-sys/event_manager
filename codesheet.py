from nicegui import ui


def show_auth_page():
    with ui.row().classes('w-full h-screen bg-gray-100'):
        
        # 🌟Left Side - Welcome text
        with ui.column().classes('w-1/2 flex flex-col justify-center items-center bg-blue-700 text-white p-12'):
            ui.label("Event Hive").classes("text-5xl font-bold mb-6")
            ui.label("Welcome Back").classes("text-2xl mb-4")
            ui.label("To keep connected with us, provide your login details").classes(
                "text-lg text-gray-200 text-center"
            )

        # 🌟 Right Side - Sign Up / Sign In Form
        with ui.card().classes('w-1/2 flex flex-col justify-center items-center bg-white shadow-xl rounded-none p-12'):
            ui.label("Sign Up to Event Hive").classes("text-3xl font-bold text-blue-700 mb-6")

            name = ui.input("Your Name", placeholder="Enter your name").classes("w-full mb-4")
            password = ui.input("Password", placeholder="Enter your password").props("type=password").classes("w-full mb-4")
            confirm_password = ui.input("Confirm Password", placeholder="Enter your password again").props("type=password").classes("w-full mb-6")

            ui.button("Sign Up", color="blue-700").classes("w-full text-white px-6 py-3 rounded-lg mb-4 font-semibold hover:bg-blue-800 transition")

            ui.label("Or").classes("text-gray-500 mb-4")
            ui.button("Sign up with Google", color="red").classes("w-full text-white px-6 py-3 rounded-lg mb-4")

            ui.separator()

            ui.label("Already have an account?").classes("text-gray-600 mt-4")
            ui.button("Sign In", color="green").classes("w-full text-white px-6 py-3 rounded-lg font-semibold hover:bg-green-800 transition")


show_auth_page()
ui.run()







main .................................from nicegui import ui,app
from pages.home import *
from pages.signin import *
from pages.signup import *
from pages.event import *
from pages.college import *
from pages.create_event import *
from pages.not_found import *
from components.navbar import *
from components.footer import *

app.add_static_files("/assets", "assets")









ui.run()






from nicegui import ui 
from components.navbar import show_navbar
from components.footer import show_footer



@ui.page("/event")
def show_event():
    show_navbar()
    with ui.element("main").classes("relative w-full max-w-[1440px] mx-auto min-h-[2428px] bg-[#F8F8FA] rounded-[20px] pb-12"):
        # Hero Section
        with ui.element().classes(
            "absolute left-[60px] right-[60px] top-[129px] h-[596px] rounded-[10px] overflow-hidden"
        ):
            # Background image
            ui.image('static/unsplash_qfWMUXDcN18.jpg').classes(
                "absolute inset-0 w-full h-full object-cover rounded-[10px]"
            )
            # Overlay
            ui.element().classes(
                "absolute inset-0 bg-[rgba(19,19,21,0.5)] rounded-[10px]"
            )
            # Back Button
            with ui.row().classes(
                "absolute left-[120px] top-[159px] w-[81px] h-[40px] bg-[#7848F4] rounded-[5px] shadow-md items-center justify-center gap-1"
            ):
                ui.icon('arrow_back').classes("text-white text-[20px]")
                ui.button('Back', on_click=lambda: ui.open('/'), color='white', text_color='white').props('flat').classes("text-white text-[16px] font-normal px-0")
            # Event Title
            ui.label("Dream world wide in jakatra").classes(
                "absolute left-[120px] top-[262px] text-white font-bold text-[64px] leading-[75px]"
            )
            # Subtitle
            ui.label("IIIT Sonepat").classes(
                "absolute left-[120px] top-[456px] text-white font-bold text-[36px] leading-[42px]"
            )
            # Short Description
            ui.label(
                "DesignHub organized a 3D Modeling Workshop using Blender on 16th February at 5 PM. "
                "The workshop taught participants the magic of creating stunning 3D models and animations using Blender. "
                "It was suitable for both beginners and experienced users. The event was followed by a blender-render competition, which added to the excitement."
            ).classes(
                "absolute left-[120px] top-[519px] text-white text-[16px] leading-[19px] w-[480px]"
            )






    show_footer()





    
from nicegui import ui 
from components.navbar import show_navbar
from components.footer import show_footer



@ui.page("/college")
def show_college():
    show_navbar()
    with ui.element("main").classes("relative w-full max-w-[1440px] mx-auto min-h-[2428px] bg-[#F8F8FA] rounded-[20px] pb-12"):
        # Hero Section
        with ui.element().classes(
            "absolute left-[60px] right-[60px] top-[129px] h-[596px] rounded-[10px] overflow-hidden"
        ):
            # Background image
            ui.image('static/unsplash_qfWMUXDcN18.jpg').classes(
                "absolute inset-0 w-full h-full object-cover rounded-[10px]"
            )
            # Overlay
            ui.element().classes(
                "absolute inset-0 bg-[rgba(19,19,21,0.5)] rounded-[10px]"
            )
            # Back Button
            with ui.row().classes(
                "absolute left-[120px] top-[159px] w-[81px] h-[40px] bg-[#7848F4] rounded-[5px] shadow-md items-center justify-center gap-1"
            ):
                ui.icon('arrow_back').classes("text-white text-[20px]")
                ui.button('Back', on_click=lambda: ui.open('/'), color='white', text_color='white').props('flat').classes("text-white text-[16px] font-normal px-0")
            # Event Title
            ui.label("Dream world wide in jakatra").classes(
                "absolute left-[120px] top-[262px] text-white font-bold text-[64px] leading-[75px]"
            )
            # Subtitle
            ui.label("IIIT Sonepat").classes(
                "absolute left-[120px] top-[456px] text-white font-bold text-[36px] leading-[42px]"
            )
            # Short Description
            ui.label(
                "DesignHub organized a 3D Modeling Workshop using Blender on 16th February at 5 PM. "
                "The workshop taught participants the magic of creating stunning 3D models and animations using Blender. "
                "It was suitable for both beginners and experienced users. The event was followed by a blender-render competition, which added to the excitement."
            ).classes(
                "absolute left-[120px] top-[519px] text-white text-[16px] leading-[19px] w-[480px]"
            )






    show_footer()





    from nicegui import ui 
from components.navbar import show_navbar
from components.footer import show_footer

@ui.page("/college")
def show_college_page():
    show_navbar()
    with ui.element("main").classes("relative w-full max-w-[1440px] mx-auto min-h-[2428px] bg-[#F8F8FA] rounded-[20px] pb-12"):
        pass
        
        # Hero Image with navigation arrows
        with ui.element("section").classes("relative mx-[60px] mt-[129px] h-[570px] rounded-[10px] overflow-hidden"):
            ui.image("assets/audience.jpg").classes("w-full h-full object-cover rounded-[10px]")
      
            # Left arrow
            ui.button(icon="chevron_left").classes(
                "absolute left-[35px] top-1/2 -translate-y-1/2 w-[50px] h-[50px] flex items-center justify-center bg-[rgba(242,242,242,0.2)] rounded-full text-white hover:bg-white/30 transition-colors"
            )
            # Right arrow
            ui.button(icon="chevron_right").classes(
                "absolute right-[35px] top-1/2 -translate-y-1/2 w-[50px] h-[50px] flex items-center justify-center bg-[rgba(242,242,242,0.2)] rounded-full text-white hover:bg-white/30 transition-colors"
            )

        # College Title
        ui.label("IIT Roorke").classes("absolute left-[120px] top-[755px] text-[36px] font-bold leading-[42px] text-black")
        # College Description
        ui.label("DesignHub organized a 3D Modeling Workshop using Blender on 16th February at 5 PM. The workshop taught participants the magic of creating stunning 3D models and animations using Blender. It was suitable for both beginners and experienced users. The event was followed by a blender-render competition, which added to the excitement. DesignHub organized a 3D Modeling Workshop using Blender on 16th February at 5 PM. The workshop taught participants the magic of creating stunning 3D models and animations using Blender. It was suitable for both beginners and experienced users. The event was followed by a blender-render competition, which added to the excitement. DesignHub organized a 3D Modeling Workshop using Blender on 16th February at 5 PM. The workshop taught participants the magic of creating stunning 3D models and animations using Blender. It was suitable for both beginners and experienced users. The event was followed by a blender-render competition, which added to the excitement. DesignHub organized a 3D Modeling Workshop using Blender on 16th February at 5 PM. The workshop taught participants the magic of creating stunning 3D models and animations using Blender. It was suitable for both beginners and experienced users. The event was followed by a blender-render competition, which added to the excitement.").classes("absolute left-[120px] right-[120px] top-[790px] h-[285px] text-[16px] leading-[19px] text-[#7E7E7E] font-roboto")

        # College Events Title
        ui.label("College Events").classes("absolute left-[120px] top-[1095px] text-[36px] font-bold leading-[42px] text-black")

        # College Events Cards (3 in a row)
        with ui.row().classes("mx-[120px] mt-8 gap-6 flex-wrap justify-center"):
            event_images = [
                "assets/audience.jpg",
                "assets/audience.jpg",
                "assets/audience.jpg"
            ]
        with ui.row().classes("absolute left-[120px] right-[120px] top-[1178px] gap-[20px] h-[400px]"):
            for i in range(3):
                with ui.card().classes("w-[320.67px] h-[330px] bg-white rounded-[10px] flex flex-col justify-between shadow-[0px_-8px_80px_rgba(0,0,0,0.07),0px_-2.92013px_29.2013px_rgba(0,0,0,0.0482987)]"):
                    # Event Image
                   
                    with ui.element("div").classes("relative  w-full rounded-md overflow-hidden"):
                        ui.image("assets/audience.jpg").classes("w-full h-full object-cover rounded-md")
                        # FREE Tag
                        with ui.element("div").classes("absolute top-[10px] left-[10px] bg-white rounded-md px-[10px] py-[5px] flex items-center justify-center"):
                            ui.label("FREE").classes("text-[10px] text-[#7848F4]")
                    # Event Title
                    ui.label("BestSeller Book Bootcamp - Lucknow").classes("px-4 mt-[6px] text-base leading-[20px] text-black font-bold")
                    # Event Time
                    ui.label("Sat, Mar 18, 9:30PM").classes("px-4 mt-[6px] text-sm leading-[16px] text-[#7848F4]")
                    # Event Location
                    ui.label("ONLINE EVENT - Attend anywhere").classes("px-4 mt-[6px] text-sm leading-[16px] text-[#7E7E7E]")

        # More College Events Cards (3 in a row)
            with ui.row().classes("mx-[120px] mt-8 gap-6 flex-wrap justify-center"):
                more_event_images = [
                "assets/audience.jpg",
                "assets/audience.jpg",
                "assets/audience.jpg"
            ]
        with ui.row().classes("absolute left-[120px] right-[120px] top-[1608px] gap-[20px] h-[400px]"):
            for i in range(3):
                with ui.card().classes("w-[320.67px] h-[330px] bg-white rounded-[10px] flex flex-col justify-between shadow-[0px_-8px_80px_rgba(0,0,0,0.07),0px_-2.92013px_29.2013px_rgba(0,0,0,0.0482987)]"):
                    # Event Image
                    
                    with ui.element("div").classes("relative h-[140px] w-full rounded-md overflow-hidden"):
                        ui.image(more_event_images[i]).classes("w-full h-full object-cover rounded-md")
                        # FREE Tag
                        with ui.element("div").classes("absolute top-[10px] left-[10px] bg-white rounded-md px-[10px] py-[5px] flex items-center justify-center"):
                            ui.label("FREE").classes("text-[10px] text-[#7848F4]")
                    # Event Title
                    ui.label("BestSeller Book Bootcamp - Lucknow").classes("px-4 mt-[6px] text-base leading-[20px] text-black font-bold")
                    # Event Time
                    ui.label("Sat, Mar 18, 9:30PM").classes("px-4 mt-[6px] text-sm leading-[16px] text-[#7848F4]")
                    # Event Location
                    ui.label("ONLINE EVENT - Attend anywhere").classes("px-4 mt-[6px] text-sm leading-[16px] text-[#7E7E7E]")

    show_footer()






    from nicegui import ui
from components.navbar import show_navbar
from components.footer import show_footer

from nicegui import ui

def show_create_event_page():
    # --- HEADER ---
    with ui.row().classes('justify-between items-center w-full p-4 shadow-sm'):
        with ui.row().classes('items-center gap-1'):
            ui.label('Event').classes('text-2xl font-bold')
            ui.label('Hive').classes('text-2xl font-bold text-purple-600')
        with ui.row().classes('gap-4'):
            ui.button('Login').props('flat')
            ui.button('Signup', color='purple', text_color='white')

    ui.space(2)

    # --- TITLE ---
    ui.label('Create Event').classes('text-2xl font-bold text-center mb-6')

    # --- FORM FIELDS ---
    Event_Title = ui.input('Event Title').props('outlined').classes('w-full mb-4')
    Event_Venue = ui.input('Event Venue').props('outlined').classes('w-full mb-4')

    with ui.row().classes('w-full gap-4 mb-4'):
        Start_Date = ui.input('Start Date').props('outlined').classes('w-full')
        End_Date = ui.input('End Date').props('outlined').classes('w-full')

    with ui.row().classes('w-full gap-4 mb-4'):
        Start_Time = ui.input('Start Time').props('outlined').classes('w-full')
        End_Time = ui.input('End Time').props('outlined').classes('w-full')

    # --- EVENT DESCRIPTION BOX ---
    with ui.card().classes('w-full max-w-3xl mx-auto mt-6 p-6 shadow-md'):
        ui.label('Event Description').classes('text-xl font-bold text-center mb-4')

        ui.textarea('Add Tags').props('outlined').classes('w-full h-24 mb-4')
        ui.textarea('Add Description').props('outlined').classes('w-full h-40 mb-4')

        ui.button('Create Event', color='purple', text_color='white').classes('w-full')

# Run page
@ui.page('/')
def index():
    show_create_event_page()

ui.run()
